...
import { BUSINESS_CATEGORY_GROUPS, type BusinessCategoryItem } from "@/lib/business-category-taxonomy";
import { type BusinessType } from "@/lib/business-types";
import { dedupeCaseInsensitive, sanitizeCustomLabel } from "@/lib/business-tags";

export type BusinessCategorySuggestion = BusinessCategoryItem & {
  groupId: string;
  groupLabel: string;
  score: number;
};
...
export function useBusinessCategoryPicker({
  ...
  const suggestions = useMemo(() => {
    ...
    return BUSINESS_CATEGORY_GROUPS.flatMap((group) =>
      ...
      .filter((result): result is BusinessCategorySuggestion => result !== null)
  ...
  const selectSuggestion = (suggestion: BusinessCategorySuggestion) => {
