    ...
    (async () => {
      ...
      if (r) {
        const { data: p } = await supabase
          .from("provinces")
          .select("code,name,slug,flag_url")
          .eq("region_code", r.code)
          .order("name");
        ...
        if (provList.length) {
          ...
          if (cityList.length) {
            ...
            const chunks: string[][] = [];
            for (let i = 0; i < cityCodes.length; i += 200)
              chunks.push(cityCodes.slice(i, i + 200));
            const all: Brgy[] = [];
      ...
      <main className="container mx-auto px-4 py-12">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{regionData?.name ?? "…"}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <LocalityFlag
            src={regionData?.flag_url}
            name={regionData?.name ?? "Region"}
            className="h-14 w-14"
          />
        ...
        <p className="mt-2 text-muted-foreground">
          Administrative region in the Philippines. Browse provinces, cities and municipalities
          below.
        </p>
        ...
        <p className="text-sm text-muted-foreground">
          Tap a province to see its cities and municipalities.
        </p>
          ...
          {perProvince.map((p) => {
            ...
            return (
              <Link
                ...
                id={p.slug}
                ref={(el) => {
                  provinceRefs.current[p.slug] = el;
                }}
              >
                <Card
                  className={cn(
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {p.cityCount} {p.cityCount === 1 ? "city" : "cities"} · {p.munCount}{" "}
                      municipalit{p.munCount === 1 ? "y" : "ies"} · {p.brgyCount.toLocaleString()}{" "}
                      barangays
                    </div>
                  </div>
                </Card>
        ...
        {perProvince.length > 0 && (
          <>
            ...
            <p className="text-sm text-muted-foreground">
              Summary table of every province in the region.
            </p>
