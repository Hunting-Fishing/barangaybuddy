    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <p className="mt-2 text-muted-foreground">
          {brgys.length} barangays · {data?.is_city ? "City" : "Municipality"}
        </p>
        <div className="mt-10 grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {brgys.map((b) => (
            <Link
              ...
              params={{ city: city, barangay: b.slug }}
            >
              <Card className="p-3 text-sm transition-all hover:shadow-soft hover:-translate-y-0.5">
                {b.name}
              </Card>
            </Link>
          ))}
