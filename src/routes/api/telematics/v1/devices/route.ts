...
async function loadDevice(deviceId: string) {
  return (supabaseAdmin as any)
    ...
    .select("id,operator_id,public_id,status")
    .eq("id", deviceId)
    .maybeSingle();
}
  ...
  server: {
    handlers: {
      ...
      PATCH: async ({ request }) => {
        ...
        if (!parsed.success) {
          return Response.json(
            { error: "Invalid device action", details: parsed.error.flatten() },
            { status: 400 },
          );
        }
        ...
        if (action === "assign_vehicle") {
          ...
          const { data: targetAssignment, error: targetError } = await (supabaseAdmin as any)
            .from("jeepney_device_assignments")
          ...
          if (targetAssignment && targetAssignment.device_id !== deviceId) {
            return jsonError("Vehicle already has an active GPS tracker", 409);
          }
          if (targetAssignment?.device_id === deviceId) {
            return Response.json({ ok: true, device_id: deviceId, vehicle_id: vehicleId, unchanged: true });
          }
          ...
          const { data: assignment, error: insertError } = await (supabaseAdmin as any)
            .from("jeepney_device_assignments")
            .insert({
              device_id: deviceId,
              vehicle_id: vehicleId,
              installation_note: note ?? "Assigned from Barangay Buddy GPS admin",
            })
            .select("id,device_id,vehicle_id,installed_at")
            .maybeSingle();
          if (insertError || !assignment) {
            return jsonError("Could not assign tracker to vehicle", 503);
          }
