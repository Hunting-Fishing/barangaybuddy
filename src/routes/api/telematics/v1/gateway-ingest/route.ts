...
export const Route = createFileRoute("/api/telematics/v1/gateway-ingest")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const source = `gateway:${String(gateway.provider).toLowerCase().replace(/[^a-z0-9_-]+/g, "-").slice(0, 40)}`;
        ...
        if (positionError || !position?.id) {
          console.error("Gateway position insert failed", positionError);
          return jsonError("Could not store normalized telemetry", 500);
        }
        ...
        if (receiptError) {
          // The position is already committed. Log receipt failure loudly so the
          // integration can be retried/inspected; sequence replay may create a
          // second position if this rare failure is not reconciled operationally.
          console.error("Gateway ingest receipt insert failed", receiptError);
        }
