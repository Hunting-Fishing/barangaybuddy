...
export const Route = createFileRoute("/fuel")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Crowdsourced live fuel prices at every station, updated by the community.",
      },
    ],
  }),
  component: FuelPage,
...
function FuelPage() {
  ...
  async function vote(id: string, v: 1 | -1) {
    ...
    if (current === v) {
      ...
      const { error } = await supabase
        .from("fuel_price_votes")
        .upsert(
          { fuel_price_id: id, user_id: user.id, vote: v },
          { onConflict: "fuel_price_id,user_id" },
        );
      if (error) return toast.error(error.message);
  ...
  async function addStation(e: React.FormEvent) {
    ...
    if (branch.length < 2)
      return toast.error('Add a branch name or location, e.g. "Shell EDSA Cubao".');
    const lat = s_lat ? Number(s_lat) : null;
    const lng = s_lng ? Number(s_lng) : null;
    if (s_lat && (Number.isNaN(lat!) || lat! < 4 || lat! > 22))
      return toast.error("Latitude looks off for the Philippines.");
    if (s_lng && (Number.isNaN(lng!) || lng! < 115 || lng! > 128))
      return toast.error("Longitude looks off for the Philippines.");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Live fuel prices, crowdsourced from the community.
            </p>
          </div>
        </div>
        <div className="mt-8">
          ...
          <Card className="p-6 lg:col-span-1">
                  ...
                  <form onSubmit={addStation} className="space-y-3">
                    ...
                    <div>
                      <Label>
                        Barangay{" "}
                        {brgy && (
                          <span className="text-xs text-muted-foreground">— {brgy.label}</span>
                        )}
                      </Label>
                      <Input
                        value={brgyQuery}
                        onChange={(e) => {
                          setBrgyQuery(e.target.value);
                          setBrgy(null);
                        }}
                          ...
                          {brgyResults.map((b: any) => (
                            <button
                              ...
                              onClick={() => {
                                setBrgy({
                                  code: b.code,
                                  label: `${b.name}, ${b.cities_municipalities?.name}`,
                                });
                                setBrgyQuery("");
                                setBrgyResults([]);
                              }}
                            >
                              ...
                              <span className="text-muted-foreground">
                                ...
                                {b.cities_municipalities?.provinces?.name}
                              </span>
                            </button>
                          ))}
                        </div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Tip: open the station on Google Maps, long-press the pin, and copy the
                      lat/long.
                    </p>
                    <DialogFooter>
            ...
            {stations.length === 0 && (
              <p className="mt-3 text-sm text-muted-foreground">
                No fuel stations registered yet. Use “Add station” to start the map.
              </p>
            )}
            ...
            <div className="mt-6 rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">
                Bulk-import station data (legal sources)
              </p>
              <p className="mt-1">
                The Philippine Department of Energy publishes the official list of Liquid Fuel
                Retail Outlets (LFROs) with valid Certificates of Compliance, plus weekly retail
                pump prices — these are government publications and free to reuse with attribution.
              </p>
              <ul className="mt-2 list-disc space-y-1 pl-4">
                <li>
                  <a
                    className="underline"
                    href="https://legacy.doe.gov.ph/downstream-oil/advisory"
                    target="_blank"
                    rel="noreferrer"
                  >
                ...
                <li>
                  <a
                    className="underline"
                    href="https://legacy.doe.gov.ph/retail-pump-price-quality-service-dashboard"
                    target="_blank"
                    rel="noreferrer"
                  >
                  ...
                  <Card key={p.id} className="flex items-center justify-between p-4">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {p.businesses?.barangays?.name} · {new Date(p.reported_at).toLocaleString()}
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="text-xs uppercase text-muted-foreground">
                          {FUEL_LABELS[p.fuel_type]}
                        </div>
