...
function BizPage() {
  ...
  async function load() {
    ...
    if (b) {
      const [{ data: l }, { data: r }] = await Promise.all([
        ...
        supabase.from("reviews").select("*, profiles(display_name)").eq("business_id", b.id).order("created_at", { ascending: false }),
      ]);
      setListings(l ?? []);
      setReviews(r ?? []);
    }
  ...
  async function submitReview(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("reviews").upsert({ business_id: biz.id, user_id: user.id, rating, comment }, { onConflict: "business_id,user_id" });
    if (error) return toast.error(error.message);
    toast.success("Review posted!"); setComment(""); load();
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="aspect-[3/1] w-full bg-gradient-hero">
        {biz.cover_image_url && <img src={biz.cover_image_url} alt="" className="h-full w-full object-cover" />}
      </div>
      <main className="container mx-auto px-4 py-12">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <span className="inline-block rounded-full bg-secondary px-3 py-1 text-xs font-medium uppercase">{biz.type.replace("_", " ")}</span>
            <h1 className="mt-3 font-display text-4xl font-bold md:text-5xl">{biz.name}</h1>
            <div className="mt-2 flex items-center gap-1 text-sm text-muted-foreground">
              <MapPin className="h-4 w-4" /> Barangay {biz.barangays?.name}, {biz.barangays?.cities_municipalities?.name}
            </div>
            <div className="mt-2 flex items-center gap-2">
              <div className="flex">{[1,2,3,4,5].map(n => <Star key={n} className={`h-4 w-4 ${n <= avg ? "fill-sun text-sun" : "text-border"}`} />)}</div>
              <span className="text-sm text-muted-foreground">{reviews.length} reviews</span>
            </div>
          </div>
          <Button onClick={startChat} className="gap-2"><MessageSquare className="h-4 w-4" /> Message owner</Button>
        </div>

        {biz.description && <p className="mt-6 max-w-3xl text-muted-foreground">{biz.description}</p>}
              ...
              <div className="flex gap-1">
                {[1,2,3,4,5].map(n => (
                  <button key={n} type="button" onClick={() => setRating(n)}>
                    <Star className={`h-6 w-6 ${n <= rating ? "fill-sun text-sun" : "text-border"}`} />
                  </button>
                ))}
              </div>
              <Textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Share your experience…" className="mt-3" />
              <Button type="submit" className="mt-3">Post review</Button>
            </form>
          )}
            ...
            {reviews.map((r) => (
              <Card key={r.id} className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex">{[1,2,3,4,5].map(n => <Star key={n} className={`h-3 w-3 ${n <= r.rating ? "fill-sun text-sun" : "text-border"}`} />)}</div>
                </div>
