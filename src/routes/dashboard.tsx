...
function Dashboard() {
  ...
  async function create(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("businesses")
      .insert({ ...parsed.data, additional_types, custom_types, tags, owner_id: user.id, slug });
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" asChild>
              <Link to="/roadsafe-operations">
                <ShieldCheck className="mr-2 h-4 w-4" /> RoadSafe operations
              </Link>
            </Button>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="font-display text-lg font-bold">
                  Admin: Auto-import businesses (OSM)
                </h2>
                <p className="text-sm text-muted-foreground">
                  Pulls Philippine businesses from OpenStreetMap and adds them as{" "}
                  <strong>unclaimed</strong> listings. Runs nightly; you can trigger it manually
                  here.
                </p>
                {lastRun && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    {lastRun.error && (
                      <span className="text-destructive"> · {lastRun.error.slice(0, 100)}</span>
                    )}
                  </p>
                )}
              </div>
            ...
            <form onSubmit={create} className="grid gap-5 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <Label>
                  ...
                  <span className="text-xs text-muted-foreground">
                    — what does the place have? (billiards, videoke, WiFi, GCash…)
                  </span>
                </Label>
                <div className="mt-2">
                  <FeatureTagsPicker
              ...
              <div className="md:col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, barangay_search: e.target.value })}
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {businesses.map((b) => (
            <Card
              key={b.id}
              className="flex flex-col p-5 transition-all hover:-translate-y-1 hover:shadow-elegant"
            >
              <div className="text-xs uppercase text-muted-foreground">
                {[
                  TYPE_LABEL[b.type as BizType] ?? b.type,
                  ...(b.additional_types ?? []).map((t: BizType) => TYPE_LABEL[t] ?? t),
                  ...(b.custom_types ?? []),
                ].join(" · ")}
              </div>
          ...
          {businesses.length === 0 && (
            <p className="text-muted-foreground">No businesses yet. Create your first one above.</p>
          )}
        </div>
      </main>
      <SiteFooter />
