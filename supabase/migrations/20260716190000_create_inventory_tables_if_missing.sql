...
create index if not exists inventory_items_business_id_idx
...
create index if not exists inventory_items_low_stock_idx
  on public.inventory_items (business_id, quantity, reorder_point);
...
create index if not exists inventory_adjustments_business_id_idx
  on public.inventory_adjustments (business_id);

create index if not exists inventory_adjustments_item_id_idx
  on public.inventory_adjustments (item_id);
...
comment on table public.inventory_items is
  'Owner-only inventory items for BarangayHub business stock control.';

comment on table public.inventory_adjustments is
  'Owner-only stock adjustment history for BarangayHub inventory items.';
